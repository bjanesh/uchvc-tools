...
if not os.path.isfile(image[0:-5]+'.txdump'):
    ...
    fullfwhm = sewpy.SEW(
        params = ["X_IMAGE","Y_IMAGE","FLAGS","NUMBER"],
    ...
    iraf.apphot.phot(image=image, coords=image[0:-5]+'.escut.pos', output=image[0:-5]+'.phot')
...
idno = np.loadtxt(image[0:-5]+'.escut.pos', usecols=(2,), dtype=int, unpack=True)
xpos, ypos = np.loadtxt(image[0:-5]+'.escut.pos', usecols=(0,1), unpack=True)

keepIndex = iraf_id - 1

xpos, ypos = xpos[keepIndex], ypos[keepIndex]
...
while nRemoved != 0:
    ...
    diffCheck = np.where(abs(np.median(diffCut)-diffCut) < 2.5*diffCut.std())#[i for i,d in enumerate(diff) if (-0.5 < d < 0.0)]
...
left_edge = np.array(zip(np.median(diffCut)-2.5*bin_stds, bin_centers))
right_edge = np.flipud(np.array(zip(np.median(diffCut)+2.5*bin_stds, bin_centers)))
