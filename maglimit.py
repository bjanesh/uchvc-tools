...
if not os.path.isfile(title_string+'_i_sh_masked.fits'):
	...
	for i in range(len(m3)) :
		...
		iraf.imutil.imreplace(images=title_string+"_i_sh_masked.fits["+repr(int(x1))+":"+repr(int(x2))+","+repr(int(y1))+":"+repr(int(y2))+"]", value=np.mean(bgmean_i))
...
if not os.path.isfile(title_string+'_g_sh_masked.fits'):
	...
	for i in range(len(m3)) :
		...
		if (y2 > 11000):
			y2 = 11000
		iraf.unlearn(iraf.imreplace)
		iraf.imutil.imreplace(images=title_string+"_g_sh_masked.fits["+repr(int(x1))+":"+repr(int(x2))+","+repr(int(y1))+":"+repr(int(y2))+"]", value=np.mean(bgmean_g))
...
iraf.photpars.setParam('apertures','1650') 
iraf.fitskypars.setParam('annulus',1700.)
#
...
iraf.ptools.txdump(textfiles='mag_est_i.dat', fields="id,mag,merr,flux,msky,stdev,rapert,xcen,ycen,ifilter,xairmass,image", expr='yes', headers='no', Stdout=txdump_out)
txdump_out.close()
...
iraf.ptools.txdump(textfiles='mag_est_g.dat', fields="id,mag,merr,flux,msky,stdev,rapert,xcen,ycen,ifilter,xairmass,image", expr='yes', headers='no', Stdout=txdump_out)
txdump_out.close()

fcirc_file = 'fc_list_old_3.0_26.07_AGC249525.dat'
...
eps_g, std_eps_g, zp_g, std_zp_g, eps_i, std_eps_i, zp_i, std_zp_i = js_calibrate(img1 = title_string+"_g_sh.fits", img2 = title_string+"_i_sh.fits")
	
mags_i, fl_i = np.loadtxt('phot_region_i.txdump',usecols=(1,3),unpack=True)
mags_g, fl_g = np.loadtxt('phot_region_g.txdump',usecols=(1,3),unpack=True)
...
flux_i = np.sum(fluxes_i)
flux_g = np.sum(fluxes_g)
...
error_i = np.sqrt(flux_i / epadu + area_i * stdev_i**2 + area_i**2 * stdev_i**2 / nsky_i)
...
error_g = np.sqrt(flux_g / epadu + area_g * stdev_g**2 + area_g**2 * stdev_g**2 / nsky_g)
merr_g = 1.0857 * error_g / flux_g

print fl_i, flux_i, merr_i
print fl_g, flux_g, merr_g

mags_i = np.hstack((-2.5*np.log10(fl_i)+2.5*np.log10(300.0), -2.5*np.log10(flux_i)+2.5*np.log10(300.0)))
mags_g = np.hstack((-2.5*np.log10(fl_g)+2.5*np.log10(300.0), -2.5*np.log10(flux_g)+2.5*np.log10(300.0)))
...
for i,mg in enumerate(g_0):
	...
	print g_mag,i_mag,g_mag-i_mag,g_abs,i_abs
