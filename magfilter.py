...
def main(argv):
	...
	for filter_sig in siglist :
		...
		i0fw = np.array(i0l)
		g0fw = np.array(g0l)
		ifwhmfw = np.array(ifl)
		gfwhmfw = np.array(gfl)
		# 
		...
		# g0 = g_i 
		# i0 = i_i
		gmifw = mu_gi*(g0fw-i0fw) + zp_gi
		imagfw = i0fw + eps_gi*(gmifw) + zp_i
		gmagfw = gmifw + imagfw

		# print i_x_fc,i_y_fc
		# gxl,gyl,gmagfw,gfwhmfw,ixl,iyl,imagfw,ifwhmfw = np.loadtxt('magfwhm_gi.txt',usecols=(0,1,2,3,4,5,6,7),unpack=True)
		# print i_ierr_fc,g_ierr_fc
		ifwhm = np.zeros_like(i_mag_fc)
		gfwhm = np.zeros_like(g_mag_fc)
		imfwhm = np.zeros_like(i_mag_fc)
		gmfwhm = np.zeros_like(g_mag_fc)
		
		for i in range(len(i_mag_fc)) :
			for j in range(len(ixl)) :
				if (abs(ixl[j]-i_x_fc[i])<5.0 and abs(iyl[j]-i_y_fc[i])<5.0) :
					# print ixl[j],i_x_fc[i],iyl[j],i_y_fc[i]
					# print i,'found fwhm match'
					ifwhm[i] = ifwhmfw[j]
					gfwhm[i] = gfwhmfw[j]
			print i_x_fc[i],i_y_fc[i],gfwhm[i],ifwhm[i]
			
		ax = plt.subplot(211)
		plt.scatter(gmagfw, gfwhmfw, edgecolors='none', label='peak points')
		plt.scatter(g_mag_fc, gfwhm, color='red', marker='o', edgecolors='none', label='peak points')
		gxfw,gyfw = np.loadtxt('escutRegion_g.txt',usecols=(0,1),unpack=True)
		plt.plot(gxfw,gyfw,linestyle='-', color='magenta')
		plt.xlabel('g')
		plt.ylabel('FWHM')
		...
		plt.xlim(15,30)
		ax = plt.subplot(212)
		plt.scatter(imagfw, ifwhmfw, edgecolors='none', label='peak points')
		plt.scatter(i_mag_fc, ifwhm, color='red', marker='o', edgecolors='none', label='peak points')
		ixfw,iyfw = np.loadtxt('escutRegion_i.txt',usecols=(0,1),unpack=True)
		plt.plot(ixfw,iyfw,linestyle='-', color='magenta')
		plt.ylim(4,15)
		plt.xlim(15,30)
		plt.xlabel('i')
		plt.ylabel('FWHM')
		...
		# ax.set_aspect(3)
		# plt.show()
		plt.savefig(title_string+'_'+repr(dm)+'_fwhmtest.pdf')
